...
module.exports = {
    ...
    add_rating: async(req, res)=>{
        ...
        pool.query('INSERT INTO ratings (produtos_produto_id, rating, comentario, users_username) VALUES ($1, $2, $3, $4) RETURNING (rating_id)', [prod_id, rating, comment, user] , (error, result)=>{
            
            if (error) {
                return res.status(500).json({err: error})
                }
                return res.status(200).json({status: 200, results: result.rows[0]["rating_id"]})
            })
    }
}
