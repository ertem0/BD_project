...
module.exports = {
    createCampanha: async(req, res)=>{
        ...
        try{
            ...
            //inserir o cupao
            await pool.query('INSERT INTO cupao (campanha_campanha_id, desconto, id_cupao) VALUES ($1, $2, $3)', [id_campanha, desconto, id_cupao])
            pool.query("COMMIT")
    ...
    subscribe: async(req, res)=>{
        ...
        try{
            ...
            for (const i in result.rows) {
                ...
                if((parseInt(dia) >= parseInt(inicio[0]) && parseInt(mes) >= parseInt(inicio[1]) && parseInt(inicio[2]) == parseInt(ano)) || (parseInt(ano) > parseInt(inicio[2]))){
                    cond+=1
                }
                //se a data for menor ou igual que o inicio soma a condicao
                if(( parseInt(dia)<= parseInt(fim[0]) &&  parseInt(mes)<= parseInt(fim[1]) && parseInt(fim[2]) == parseInt(ano)) || parseInt(ano) < parseInt(fim[2])){
                    cond+=1
                }
                ...
                if(cond == 2){
                    ...
                    //insere a subscricao
                    await pool.query('INSERT INTO subscricoes (posicao, comprador_users_username, cupao_id_cupao) VALUES ($1, $2, $3)', [posicao,tokeninfo.username , cupao_id.rows[0].id_cupao])
                    //atualiza o stock
